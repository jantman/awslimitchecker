"""
awslimitchecker/services/cognito.py
...
class _CognitoService(_AwsService):

    service_name = "Cognito"
    api_name = "cognito-idp"  # AWS API name to connect to (boto3.client)
    quotas_service_code = "cognito-idp"
    ...
    def find_usage(self):
        ...
        for lim in self.limits.values():
            lim._reset_usage()
        self.limits['User Pools']._add_current_usage(
            len(
                paginate_dict(
                    self.conn.list_user_pools,
                    MaxResults=60,
                    alc_marker_path=['NextToken'],
                    alc_data_path=['UserPools'],
                    alc_marker_param='NextToken',
                )["UserPools"]
            ),
            aws_type='AWS::Cognito::UserPool',
        )
    ...
    def get_limits(self):
        ...
        limits["User Pools"] = AwsLimit(
            "User pools per account",
            self,
            1000,
            self.warning_threshold,
            self.critical_threshold,
            limit_type="AWS::Cognito::UserPool",
            quotas_name="User pools per account",
        )
