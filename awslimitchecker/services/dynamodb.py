...
class _DynamodbService(_AwsService):
    ...
    def _find_usage_dynamodb(self):
        ...
        for table in self.connect().list_tables()['TableNames']:
            ...
            self.limits['Global Secondary Indexes (table)']._add_current_usage(
                len(UsagePerTable['GlobalSecondaryIndexes'] if 'GlobalSecondaryIndexes' in UsagePerTable else 0),
                resource_id=table_Arn, aws_type='AWS::DynamoDB::Table'
            )
            self.limits['Local Secondary Indexes (table)']._add_current_usage(
                len(UsagePerTable['LocalSecondaryIndexes'] if 'LocalSecondaryIndexes' in UsagePerTable else 0),
                resource_id=table_Arn, aws_type='AWS::DynamoDB::Table'
            )
